...
- 기획서(단일 원천): `docs/traffic-predict_기획서.md` (동결 v2.10). 기획서와 다르게 바꿔야 하면 코드를 고치기 전에 변경 통제 표로 영향을 보고합니다.
- 프로필: 데이터·ML 실험
...
- test(12/16~22)는 사용자가 "설정 동결, test 실행"이라고 지시하기 전까지 어떤 용도로도 쓰지 않는다.
  (2026-09-23 사용자 결정으로 범위 명확화) "쓴다" = 모델 입력·학습·채점·설정 선택·결정 지표·그래프·결과 확인에 test 값을 넣거나, test 값을 사람이나 에이전트가 들여다보는 것. 파이프라인이 계열을 만들며 test 행을 메모리에 읽었다가 잘라내는 것(`prepare`의 계열 생성·결측 검사, `run`·`results`의 계열 로드)은 여기에 들지 않는다. 단, 그 값을 출력·요약·분석하지 않는다.
...
## 핵심 원칙
- test 구간(12/16~12/22)의 y로 예측·채점하는 것은 `test` 명령뿐입니다. `prepare`는 계열 생성과 결측 검사를 위해 test 행까지 만들고, `run`·`results`는 계열을 읽은 뒤 val 끝에서 잘라 씁니다. test 값은 모델 입력·채점·선택·결정 지표·그래프에 들어가지 않습니다.
