...
| F-14 | (v2.10) 켜기/끄기 시뮬레이션 | 저장된 val 예측으로 용량 셀 켜짐/꺼짐을 칸마다 계산(3.3 "켜기/끄기 시뮬레이션"). 후보 조합(`hold_min` × `on_ratio`)을 지연(`delay_min`)·계열·거리·구역마다 계산하고, 선택 규칙 r1로 (계열, 거리, 지연)마다 한 조합을 고름 | (1) 손으로 계산한 예제와 일치(최소 유지, 켜기 기준 비율, 지연 때문에 한 칸 늦게 켜짐, 놓친 시간·횟수, 켜 둔 시간, 켜고 끈 횟수) (2) H=0분·1.0θ·지연 0분이면 켜짐 칸이 3단계 경보 칸과 같음 (3) r1: 구역마다 놓친 시간이 기준(H=0·1.0θ) 이하인 조합 중 켜 둔 시간 합 최소 → 켜고 끈 횟수 합 최소 → 작은 H → 큰 비율. 기준은 항상 후보 (4) LSTM은 시드별로 계산해 평균±표준편차, 선택은 평균으로 (5) `results`가 `results/switching.csv`(모든 조합·기준선)와 `results/switching.md`(`report_horizon`, 지연별 선택 조합과 기준선)를 재학습 없이 생성 (6) 설정 값이 틀리면 E-2001 | E-2001 |
...
| M-19 | (v2.10) `eval/switching.py: simulate(y_pred, imputed, thr, horizon, hold_min, on_ratio, delay_min)`, `switching_metrics(state, y_true, imputed, thr, merge_gap)`, `always_on(n)`, `oracle(y_true, imputed, thr)`, `choose_r1(rows)` | 3.3 켜기/끄기 시뮬레이션: 칸별 켜짐 상태, 지표, 기준선, r1 선택 | D-07 + θ → D-16 | M-18 |
