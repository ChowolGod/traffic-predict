...
- 남은 일: 목적 단계 4(최소 유지 시간 등 결정 값은 후보와 근거를 제시해 사용자가 정함) → F-09 개정(아래 F-09 참고) → 12 → 13. 그 밖에 검토에서 나온 코드 정리(선택 규칙 함수 일원화, `results`의 손상 폴더 처리, test 행 읽기 범위)는 사용자 지시 대기
