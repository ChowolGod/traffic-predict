...
- 최근 변경: v2.12(2026-09-23) F-09 개정(T1~T8 모두 추천안, 사용자 결정): test는 거리 10·30·60분 × 계열 5개(lag-1008 추가) × 구역 3개. 신뢰구간은 어제 같은 시각과 마지막 관측값 두 기준 대비, LSTM은 시드별 절대오차를 칸마다 평균해 부트스트랩. 결정 지표(3단계)와 켜기/끄기(4단계, val에서 s1로 고른 조합)를 test에 포함, 신뢰구간 없이 횟수 보고. 실험 선택은 기존 규칙(val MAE), `final.yaml`은 사람이 쓰고 코드가 검증. v2.11(2026-09-23) 4단계 선택 규칙 r1 → s1(사용자 결정): 구역마다 켜 둔 시간이 기준(H 0분·1.0θ)의 1.2배 이하인 조합 중 세 구역 놓친 혼잡 시간 합 최소 → 켜 둔 시간 합 → 켜고 끈 횟수 합 → 작은 H → 큰 비율. r1은 모든 후보의 켜짐 칸이 기준의 상위 집합이라 구조적으로 항상 기준을 골랐음(val 결과와 무관한 결함). 새 규칙은 어떤 조합을 고를지 보기 전에 정함. v2.10(2026-09-23) F-14(켜기/끄기 시뮬레이션) 정의: 지연 적용은 A(예측은 입력 마지막 칸이 끝난 t − h + 1에 나옴, 4단계에만 적용), 지연값마다 따로 선택·보고. `decision.yaml`에 `switching` 키 3개(사용자 승인), M-19, D-16. v2.9(2026-09-23) 4단계 후보·조건: 최소 유지 시간 H ∈ {0, 30, 60, 120}분, 켜기 기준 ∈ {1.0, 0.9, 0.8}×θ(모델마다 12조합), 평가 거리 10·30·60분 모두(60분 대표), 지표는 가중 없이 따로 보고하고 '항상 켬'·'미래를 아는 경우' 기준선 포함. 켜는 데 걸리는 시간은 0·10분 둘 다(시각 규칙 확인 대기). v2.8(2026-09-23) r1 세부: (7-3) '나빠지지 않음'은 세 구역 각각에서 놓친 혼잡 시간 ≤ 기준 (7-4) LSTM은 시드별로 계산한 놓친 시간·켜 둔 시간·켜고 끈 횟수를 평균해 비교. v2.7(2026-09-23) 4단계 선택 규칙: (7-1) r1 — 놓친 혼잡 시간이 비교 기준(H=0분, 켜기 기준 1.0θ = 3단계 경보)보다 나빠지지 않는 조합 중 켜 둔 시간이 가장 짧은 것, 같으면 켜고 끈 횟수가 적은 것 (7-2) 모델별 1개를 골라 세 구역에 공통 적용. 따라서 후보 목록에 H=0분·1.0θ는 반드시 포함. v2.6(2026-09-23) 목적 단계 4(켜기/끄기 시뮬레이션) 결정 2건: (1) 규칙 모양 a — 예측 ≥ 켜기 기준이면 켜고, 켠 뒤 최소 H분 유지, 그 뒤 예측이 기준 아래면 끔 (7) 절차 — 후보 조합 목록을 돌리기 전에 고정하고 val 결과를 전부 보고한 뒤 미리 정한 규칙으로 하나를 고르며 test는 그 설정으로 1회. 나머지 값(H 후보, 켜기 기준 후보, 켜는 데 걸리는 시간, 평가 거리, 지표 묶는 법, 고르는 규칙)은 결정 대기. F-14는 이 값들이 정해진 뒤 정의. v2.5(2026-09-23) test 차단 규칙의 범위를 실제 동작에 맞게 명확화(사용자 결정, 코드 변경 없음): test 값으로 예측·채점하는 것은 `test`뿐이고, test 값은 모델 입력·채점·선택·결정 지표·그래프에 들어가지 않음. 파이프라인이 계열을 만들거나 읽을 때 test 행을 메모리에 올렸다 잘라내는 것은 허용하되 그 값을 출력·요약하지 않음. v2.4(2026-09-23) 문구 정리(동작·결정 변화 없음): 문서 사이 불일치 검토 결과 반영. 구현 단위 시그니처·오류 사용처·파일 필드를 실제 코드에 맞춤, full 실험은 사용자 승인 후 실행으로 3.1을 CLAUDE.md와 맞춤, v1 표현(1스텝·보간) 정리, "단계" 용어 구분. v2.3(2026-09-23) 결정 6건 확정: (a) 기준선 이름 '마지막 관측값(10분 뒤 예측에서는 10분 전 값)' (b) 경보 = 예측 ≥ 임계치 유지 (c) 혼잡 시작 직전 h칸 안의 경보는 불필요 경보에서 제외 (d) LSTM 결정 지표는 시드별 계산 후 평균±표준편차 (e) 30·60분은 10분 뒤 예측에서 고른 하이퍼파라미터(차수·창 등) 재사용 (f) 채운 칸은 경보·혼잡에서 제외. v2.2(2026-09-23) 문구 수정: '셀 단위' → '격자 칸(약 235m) 단위, 지역별 수요 예측'(데이터 해석 주의 반영, 설계 영향 없음). v2.1(2026-09-23) 탐지 조건을 '발행 시각 < 혼잡 시작'으로 엄격화, 리드타임 10~60분. v2(2026-09-23) 목적 추가(무선망 운용 결정), F-12 예측 거리·F-13 결정 지표 추가, 제외 목록에서 '여러 스텝 앞 예측'·'lag-1008' 되살림. test(F-09) 형식은 4단계 변경 통제에서 개정 예정
