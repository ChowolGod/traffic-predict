...
## 다음 세션에게
- 현재 단계: 진행표 1~11, 14~17 완료(2026-09-23). 목적 단계 2(예측 거리)·3(결정 지표)을 val로 끝냄. 목적 단계 4(켜기/끄기, F-14)와 F-09 개정(25)까지 완료. 다음은 `final.yaml` 작성 → 12(test, "설정 동결, test 실행" 지시 후) → 13(README)
...
| 13 | F-10 | README 결과·한계 정리 | 완료 | 2026-09-23. `README.md`: test 결과(거리별 상대 MAE, 마지막 관측값 대비 95% 구간, 60분 뒤 켜기/끄기), 데이터·방법, 한계 5항목, 재현 절차(명령·시드·기대 지표), 실험 58개 표(폴더 링크·바꾼 요소·이유). 수치는 결과 파일에서 스크립트로 생성. 테스트(상대 링크가 모두 존재, 실험 링크 58개, 한계 5항목) 추가, pytest 263 통과 |
