...
상태: 동결 (2026-09-23, v2.12)
프로필: 데이터·ML 실험

## 다음 세션에게
- 현재 단계: 전 기능 완료(진행표 1~26, 2026-09-23). test는 한 번 실행되어 잠김(`results/test/LOCK`, 다시 실행하지 않음). README(F-10)와 사후 민감도 분석(26)까지 완료. 남은 일은 제외 목록의 향후 확장뿐
...
- 확정된 결정:
  ...
  - (v2.6) 4단계 규칙: 예측 ≥ 켜기 기준이면 용량 셀을 켜고 최소 H분 유지, 그 뒤 예측이 기준 아래면 끔(끄기 기준 별도 없음, 최소 꺼짐 시간 없음). 후보 조합은 실행 전에 고정하고 val로 전부 보고한 뒤 미리 정한 규칙으로 하나를 골라 test 1회
  - (v2.7) 고르는 규칙 r1: 모델마다, 놓친 혼잡 시간이 그 모델의 비교 기준(H=0분·1.0θ)보다 나빠지지 않는 조합 중 켜 둔 시간 최소 → 켜고 끈 횟수 최소. 고른 조합을 세 구역에 공통 적용
  - (v2.11) **위 r1은 s1로 대체**: 구역마다 켜 둔 시간 ≤ 1.2 × 기준(H 0분·1.0θ)인 조합 중 세 구역 놓친 혼잡 시간 합 최소 → 켜 둔 시간 합 최소 → 켜고 끈 횟수 합 최소 → 작은 H → 큰 비율. 기준은 항상 후보. 여유 20%는 `eval/switching.py`의 상수 `ON_TIME_ALLOWANCE = 0.2`(설정 키 아님). LSTM은 시드 평균, 모델별 1개·세 구역 공통은 그대로
  - (v2.8) '나빠지지 않음'은 구역마다 따로 판정(세 구역 모두 놓친 시간 ≤ 기준). LSTM은 시드별 지표의 평균으로 비교
  - (v2.9) 후보: H ∈ {0, 30, 60, 120}분 × 켜기 기준 ∈ {1.0, 0.9, 0.8}×θ. 거리 10·30·60분 모두 계산(60분 대표). 지표(놓친 혼잡 시간·횟수, 켜 둔 시간, 켜고 끈 횟수)는 따로 보고, 기준선 '항상 켬'·'미래를 아는 경우' 포함. 켜는 데 걸리는 시간 0·10분 둘 다
  - (v2.10) 지연 적용 A: 4단계에서는 예측이 입력 마지막 칸이 끝난 시각(t − h + 1)에 나온다고 봄(3단계 리드타임은 t − h 기준이라 10분 차이, 보고서에 명시). 지연은 셀 환경 가정이라 r1로 고르지 않고 지연값마다 따로 선택·보고
- 남은 일: 없음(Must·Should 모두 완료, test 잠김). 향후 확장은 제외 목록(절전 시나리오 등) 참고
- 제약: 마감 없음 · 채점 기준 없음 · 제출물 = GitHub 저장소 README · 구현함
- 최근 변경 (최신이 위, 동결 후 변경 통제로 바뀐 것):
  - **v2.12** (2026-09-23): F-09 개정(T1~T8 모두 추천안, 사용자 결정): test는 거리 10·30·60분 × 계열 5개(lag-1008 추가) × 구역 3개. 신뢰구간은 어제 같은 시각과 마지막 관측값 두 기준 대비, LSTM은 시드별 절대오차를 칸마다 평균해 부트스트랩. 결정 지표(3단계)와 켜기/끄기(4단계, val에서 s1로 고른 조합)를 test에 포함, 신뢰구간 없이 횟수 보고. 실험 선택은 기존 규칙(val MAE), `final.yaml`은 사람이 쓰고 코드가 검증.
  - **v2.11** (2026-09-23): 4단계 선택 규칙 r1 → s1(사용자 결정): 구역마다 켜 둔 시간이 기준(H 0분·1.0θ)의 1.2배 이하인 조합 중 세 구역 놓친 혼잡 시간 합 최소 → 켜 둔 시간 합 → 켜고 끈 횟수 합 → 작은 H → 큰 비율. r1은 모든 후보의 켜짐 칸이 기준의 상위 집합이라 구조적으로 항상 기준을 골랐음(val 결과와 무관한 결함). 새 규칙은 어떤 조합을 고를지 보기 전에 정함.
  - **v2.10** (2026-09-23): F-14(켜기/끄기 시뮬레이션) 정의: 지연 적용은 A(예측은 입력 마지막 칸이 끝난 t − h + 1에 나옴, 4단계에만 적용), 지연값마다 따로 선택·보고. `decision.yaml`에 `switching` 키 3개(사용자 승인), M-19, D-16.
  - **v2.9** (2026-09-23): 4단계 후보·조건: 최소 유지 시간 H ∈ {0, 30, 60, 120}분, 켜기 기준 ∈ {1.0, 0.9, 0.8}×θ(모델마다 12조합), 평가 거리 10·30·60분 모두(60분 대표), 지표는 가중 없이 따로 보고하고 '항상 켬'·'미래를 아는 경우' 기준선 포함. 켜는 데 걸리는 시간은 0·10분 둘 다(시각 규칙 확인 대기).
  - **v2.8** (2026-09-23): r1 세부: (7-3) '나빠지지 않음'은 세 구역 각각에서 놓친 혼잡 시간 ≤ 기준 (7-4) LSTM은 시드별로 계산한 놓친 시간·켜 둔 시간·켜고 끈 횟수를 평균해 비교.
  - **v2.7** (2026-09-23): 4단계 선택 규칙: (7-1) r1 — 놓친 혼잡 시간이 비교 기준(H=0분, 켜기 기준 1.0θ = 3단계 경보)보다 나빠지지 않는 조합 중 켜 둔 시간이 가장 짧은 것, 같으면 켜고 끈 횟수가 적은 것 (7-2) 모델별 1개를 골라 세 구역에 공통 적용. 따라서 후보 목록에 H=0분·1.0θ는 반드시 포함.
  - **v2.6** (2026-09-23): 목적 단계 4(켜기/끄기 시뮬레이션) 결정 2건: (1) 규칙 모양 a — 예측 ≥ 켜기 기준이면 켜고, 켠 뒤 최소 H분 유지, 그 뒤 예측이 기준 아래면 끔 (7) 절차 — 후보 조합 목록을 돌리기 전에 고정하고 val 결과를 전부 보고한 뒤 미리 정한 규칙으로 하나를 고르며 test는 그 설정으로 1회. 나머지 값(H 후보, 켜기 기준 후보, 켜는 데 걸리는 시간, 평가 거리, 지표 묶는 법, 고르는 규칙)은 결정 대기. F-14는 이 값들이 정해진 뒤 정의.
  - **v2.5** (2026-09-23): test 차단 규칙의 범위를 실제 동작에 맞게 명확화(사용자 결정, 코드 변경 없음): test 값으로 예측·채점하는 것은 `test`뿐이고, test 값은 모델 입력·채점·선택·결정 지표·그래프에 들어가지 않음. 파이프라인이 계열을 만들거나 읽을 때 test 행을 메모리에 올렸다 잘라내는 것은 허용하되 그 값을 출력·요약하지 않음.
  - **v2.4** (2026-09-23): 문구 정리(동작·결정 변화 없음): 문서 사이 불일치 검토 결과 반영. 구현 단위 시그니처·오류 사용처·파일 필드를 실제 코드에 맞춤, full 실험은 사용자 승인 후 실행으로 3.1을 CLAUDE.md와 맞춤, v1 표현(1스텝·보간) 정리, "단계" 용어 구분.
  - **v2.3** (2026-09-23): 결정 6건 확정: (a) 기준선 이름 '마지막 관측값(10분 뒤 예측에서는 10분 전 값)' (b) 경보 = 예측 ≥ 임계치 유지 (c) 혼잡 시작 직전 h칸 안의 경보는 불필요 경보에서 제외 (d) LSTM 결정 지표는 시드별 계산 후 평균±표준편차 (e) 30·60분은 10분 뒤 예측에서 고른 하이퍼파라미터(차수·창 등) 재사용 (f) 채운 칸은 경보·혼잡에서 제외.
  - **v2.2** (2026-09-23): 문구 수정: '셀 단위' → '격자 칸(약 235m) 단위, 지역별 수요 예측'(데이터 해석 주의 반영, 설계 영향 없음).
  - **v2.1** (2026-09-23): 탐지 조건을 '발행 시각 < 혼잡 시작'으로 엄격화, 리드타임 10~60분.
  - **v2** (2026-09-23): 목적 추가(무선망 운용 결정), F-12 예측 거리·F-13 결정 지표 추가, 제외 목록에서 '여러 스텝 앞 예측'·'lag-1008' 되살림. test(F-09) 형식은 4단계 변경 통제에서 개정 예정
- 마지막 실행: test 1회(2026-09-23, `final.yaml` 45칸, 커밋 d163b69). 구역 평균 상대 MAE 60분 뒤 LSTM 0.521 · ARIMA 0.687 · 마지막 관측값 0.914. 60분 뒤 켜기/끄기는 놓침 비용 비율 w 약 1.6~4.7에서 LSTM이 val·test 모두 1위(사후 분석). pytest 264 통과
...
| F-09 | test 1회 평가 | (v2.12) 전체 기간에서 (구역, 거리 10·30·60분, 계열 5개: lag-144·lag-1·lag-1008·ARIMA·LSTM)마다 `final.yaml`의 실험을 한 번에 test로 평가하고 잠금. 예측 오차, 결정 지표(3.3), 켜기/끄기(3.3, val에서 s1로 고른 조합)를 함께 계산 | (1) 두 번째 실행은 오류로 거부됨 (2) `--confirm`이 없거나 dirty면 거부됨 (3) test 결과 표에 거리별 MAE·RMSE·상대 MAE와, 어제 같은 시각·마지막 관측값 각각 대비 MAE 차이의 95% 신뢰구간이 있음 (4) 부트스트랩이 같은 시드에서 같은 구간을 냄 (5) 선택 규칙의 후보는 같은 (구역, 거리, 계열)의 실험, 실험의 horizon이 키와 다르면 거부 (6) LSTM 신뢰구간은 시드별 절대오차를 칸마다 평균한 값으로 계산(보고 MAE와 같은 양) (7) 결정 지표는 train 임계치로, LSTM은 시드별 평균±표준편차 (8) 켜기/끄기는 val에서 (계열, 거리, 지연)마다 s1로 고른 조합을 test에 그대로 적용하고 기준선 두 개를 함께 보고(test에서 다시 고르지 않음) | E-2001, E-4005, E-4006 |
...
| F-14 | (v2.10) 켜기/끄기 시뮬레이션 | 저장된 val 예측으로 용량 셀 켜짐/꺼짐을 칸마다 계산(3.3 "켜기/끄기 시뮬레이션"). 후보 조합(`hold_min` × `on_ratio`)을 지연(`delay_min`)·계열·거리·구역마다 계산하고, 선택 규칙 s1(v2.11, r1 대체)로 (계열, 거리, 지연)마다 한 조합을 고름 | (1) 손으로 계산한 예제와 일치(최소 유지, 켜기 기준 비율, 지연 때문에 한 칸 늦게 켜짐, 놓친 시간·횟수, 켜 둔 시간, 켜고 끈 횟수) (2) H=0분·1.0θ·지연 0분이면 켜짐 칸이 3단계 경보 칸과 같음 (3) (v2.11) s1: 구역마다 켜 둔 시간이 기준(H=0·1.0θ)의 1.2배 이하인 조합 중 놓친 시간 합 최소 → 켜 둔 시간 합 최소 → 켜고 끈 횟수 합 최소 → 작은 H → 큰 비율. 기준은 항상 후보 (4) LSTM은 시드별로 계산해 평균±표준편차, 선택은 평균으로 (5) `results`가 `results/switching.csv`(모든 조합·기준선)와 `results/switching.md`(`report_horizon`, 지연별 선택 조합과 기준선)를 재학습 없이 생성 (6) 설정 값이 틀리면 E-2001 | E-2001 |
...
| 켜기/끄기 시뮬레이션 (v2.10) | 대상: (계열, 거리 h, 구역)마다 D-15 선택 규칙상 최선 실험의 val 예측, 칸은 시간순, 처음 상태는 꺼짐. **시각**: ŷ_t는 t − h + 1에 나옴. 지연 d칸(`delay_min`/10)이면 ŷ로 켠 명령은 d칸 뒤에 반영되므로, 칸 t에 쓸 수 있는 가장 새 예측은 ŷ_{t − L}, L = max(0, d + 1 − h). **원함** want(t) = ŷ_{t − L} ≥ `on_ratio` × θ 이고 t가 채운 칸이 아님(t − L < 첫 칸이면 꺼짐 원함). **상태**: 꺼짐이고 want면 켬(켜고 끈 횟수 +1, 켠 칸 기록). 켜짐이면 켠 뒤 `hold_min`/10칸 동안은 유지하고, 그 뒤 want가 아니면 끔. **지표**: 놓친 혼잡 시간 = 혼잡 칸(y ≥ θ, 채운 칸 제외) 중 꺼짐 × 10분. 놓친 혼잡 횟수 = 혼잡 구간(3.3 병합 규칙) 중 시작 칸이 꺼짐인 수. 켜 둔 시간 = 켜짐 칸 × 10분. 켜고 끈 횟수 = 꺼짐→켜짐 전환 수. **기준선**: 항상 켬(모든 칸 켜짐), 미래를 아는 경우(혼잡 칸에만 켜짐). **선택 s1**(v2.11, r1 대체): (계열, 거리, 지연)마다 기준 = H 0분·1.0θ. 세 구역 **각각** 켜 둔 시간 ≤ 1.2 × 기준인 조합 중 세 구역 놓친 시간 합 최소 → 켜 둔 시간 합 최소 → 켜고 끈 횟수 합 최소 → 작은 H → 큰 비율. LSTM은 시드별 지표 평균으로 비교. 고른 조합은 세 구역 공통 | 혼잡을 미리 막는 운용을 저장된 예측으로 재현. 놓침이 더 비싸다는 목적을 가중치 없이 반영 |
...
| 불확실성(test) | (v2.12) 계열·거리마다 "모델 MAE − 기준 MAE"의 95% 신뢰구간을 **어제 같은 시각(lag-144)**과 **마지막 관측값(lag-1)** 두 기준에 대해 **일별 블록 부트스트랩**으로 계산(블록 = Europe/Rome 하루 144칸, 반복 2000, 시드 0, 백분위수 방식). 기준은 같은 거리의 해당 계열 실험. LSTM은 칸마다 시드별 절대오차의 평균을 오차로 씀(보고 MAE = 시드별 MAE 평균과 같은 양, 앙상블 이점 없음). 결정 지표·켜기/끄기는 구간 없이 값만 보고. test가 7일이라 블록이 7개뿐이어서 구간이 거칠다는 점을 한계로 적음 | test 1주 단일 구간에서 우연한 차이인지 판단. 짧은 거리에서 가장 강한 기준(lag-1)에도 이기는지 확인 |
...
| I-05 | `test --final configs/final.yaml --confirm` | F-09 | 최종 설정(D-06), (v2.12) `configs/decision.yaml` | `results/test/metrics.csv·md`, `switching.csv`, `predictions.parquet`, `LOCK` (D-10) | M-14 → M-05, M-07~M-10, M-13, M-17, M-18, M-19 | E-2001, E-2003, E-4004, E-4005, E-4006, E-4007 |
...
| M-12 | `exp/results.py: rebuild_results()` | 결과 표·그림 재생성. (v2) horizon 열, 결정 지표 열(M-18), 거리별 표·그래프(D-15) | 실험 폴더들 → D-09, D-15, (v2.10) D-16 | M-04, M-10, M-15, M-18, M-19 |
...
| M-19 | (v2.10) `eval/switching.py: simulate(y_pred, imputed, thr, horizon, hold_min, on_ratio, delay_min)`, `switching_metrics(state, y_true, imputed, thr, merge_gap)`, `always_on(n)`, `oracle(y_true, imputed, thr)`, `choose_s1(rows)` | 3.3 켜기/끄기 시뮬레이션: 칸별 켜짐 상태, 지표, 기준선, s1 선택 | D-07 + θ → D-16 | M-18 |
| M-13 | `seed.py: set_seed(seed, threads)` | 3.3의 시드 정책 적용 | — | — |
| M-14 | `exp/testrun.py: run_test(final_path, confirm)` | 최종 설정 검증(선택 규칙 포함), 각 실험의 D-11 모델을 불러와(재학습 없음) test 구간 1회 평가, 두 기준 대비 부트스트랩 신뢰구간, (v2.12) 결정 지표, val에서 s1로 고른 켜기/끄기 조합 적용, 원자적 결과 쓰기(4.1) | D-06, D-11 → D-10 | M-05, M-07~M-10, M-12(결정·켜기/끄기 계산 함수), M-13, M-17, M-18, M-19 |
...
| E-2001 | 설정 파일 오류. 단계 설정(날짜 순서, 구간 겹침, dev에 test, 허용 범위 11/01~12/22 밖, K가 1~3이 아님), (v2) 결정 설정(`threshold_ratio`가 (0, 1] 밖, 즉 0 이하 또는 1 초과, `peak_quantile` 0.5~1 밖, `merge_gap` 0~6 밖, `report_horizon`이 1·3·6이 아님, (v2.10) `switching.hold_min`이 0을 포함하지 않거나 0~240 밖·10의 배수 아님, `switching.on_ratio`가 1.0을 포함하지 않거나 (0, 1] 밖, `switching.delay_min`이 0~60 밖·10의 배수 아님, 목록이 비었거나 중복, 키 누락) | `[E-2001] 분할 설정 오류: <내용>` / `[E-2001] 결정 설정 오류: <내용>` | M-15 (I-01, I-02, I-04) | 중단 |
...
- D-06 `configs/final.yaml` (v2.12): `{zones: {<zone_rank>: {<horizon>: {naive_144: EXP-###, naive_1: EXP-###, naive_1008: EXP-###, arima: EXP-###, lstm: EXP-###}}}}`. zone_rank 키는 1~K(full의 K와 같음), horizon 키는 1·3·6 모두, 계열 키는 다섯 개 모두. 각 실험은 그 구역·거리·계열의 D-15 선택 규칙상 최선이어야 하고 horizon이 키와 같아야 함. 검증은 E-4006. 사람이 `results/horizon.md`를 보고 작성해 커밋하고, `test`는 규칙과 다르면 거부. F-11에서 구역 2·3의 실험은 구역 1의 실험을 부모로 두고 `changed: zone_rank`로 만듦(한 요소 규칙 유지)
...
**D-10 test** (`results/test/`, v2.12): `metrics.csv·md`(행: 계열 × zone_rank × horizon. 열: `family, zone_rank, square_id, horizon, mae, mae_std, rmse, rmse_std, rel_mae, diff_vs_lag144, ci_lag144_low, ci_lag144_high, diff_vs_lag1, ci_lag1_low, ci_lag1_high, dec_threshold, dec_episodes, dec_missed, dec_missed_std, dec_false_alarm_min, dec_false_alarm_min_std, dec_lead_min, dec_lead_min_std`. LSTM의 mae·rmse·dec_*는 시드별 평균±표준편차. F-11이면 (계열, 거리)마다 구역 평균 행 추가(rel_mae 평균, zone_rank=`mean`)), `switching.csv`(D-16과 같은 열에서 `exp_id`·`chosen` 대신 val에서 고른 조합만, 기준선 행 포함), `predictions.parquet`(`family, zone_rank, square_id, horizon, time_utc, y_true, y_pred, seed, is_imputed`), `LOCK`(`{evaluated_at, final: {zone: {horizon: {family: EXP}}}, git_commit}`).
...
**D-14 결정 설정** (v2, `configs/decision.yaml`): `{threshold_ratio: 0.7, peak_quantile: 0.99, merge_gap: 1, report_horizon: 6, switching: {hold_min: [0, 30, 60, 120], on_ratio: [1.0, 0.9, 0.8], delay_min: [0, 10]}}`((v2.10) `switching`은 4단계 후보, 사용자 승인). 검증은 E-2001. `report_horizon`은 결정 지표를 대표로 보고하는 거리.
**D-16 켜기/끄기 결과** (v2.10, `results/switching.csv·md`): csv 열 `family, zone_rank, square_id, exp_id, horizon, delay_min, hold_min, on_ratio, episodes, missed_min, missed_min_std, missed_episodes, missed_episodes_std, on_min, on_min_std, switches, switches_std, chosen`. 기준선 행은 `family`가 `always_on`·`oracle`이고 `hold_min`·`on_ratio`·`exp_id`는 빈칸. `_std`는 LSTM 시드 간 표준편차(ddof=1), 다른 계열은 빈칸. md는 `report_horizon`에서 지연값마다 계열별 선택 조합과 구역별 지표, 기준선.
...
```
traffic-predict/
├─ pyproject.toml, uv.lock, README.md (F-10), CLAUDE.md, .gitignore
├─ docs/traffic-predict_기획서.md
├─ configs/ phases.yaml, decision.yaml (v2), final.yaml (test 목록), experiments/EXP-###.yaml
...
│   ├─ eval/ metrics.py, bootstrap.py, decision.py (v2), switching.py (v2.10)
│   └─ exp/ registry.py, results.py, testrun.py, selection.py (3.3 선택 규칙, results·test 공용)
├─ scripts/ make_readme.py (README를 결과 파일에서 생성, 테스트가 일치 확인)
├─ tests/ fixtures/, test_*.py
├─ data/        (git 제외) raw/, interim/daily/, processed/<phase>/
├─ experiments/ (커밋) EXP-###_<name>/
└─ results/     (커밋) results.csv·md, horizon.md (v2), switching.csv·md (v2.10), figures/, test/
```
...
| (v2.6) 4단계 켜기/끄기 규칙은 '켜기 기준 + 최소 유지 시간 H'(a) | 켜기·끄기 기준 분리(b), 최소 꺼짐 시간 추가(c) | 가장 단순하고 CLAUDE.md의 최소 유지 시간 규칙과 일치. val에서 잦은 켜고 끔은 주로 5161·단순 기준선에서만 보임(2026-09-23 사용자 결정) |
| (v2.6) 4단계 값은 후보 조합을 먼저 고정하고 val 전체 보고 후 미리 정한 규칙으로 선택 | 값 하나만 미리 정하고 1회 실행 | 비교 근거를 남기면서도 val을 본 뒤 후보를 늘리거나 규칙을 바꾸는 일을 막음. test는 선택된 설정으로 1회(사용자 결정) |
| (v2.7) 고르는 규칙 r1(놓침이 기준보다 나빠지지 않는 조합 중 켜 둔 시간 최소), 모델별 1개 | r2(놓침 최소), r3(켜고 끈 횟수 최소) / 전체 공통 1개, 모델·구역별 | 놓침이 더 비싸다는 목적을 지키며 비용 비율 없이 낭비를 줄임. 모델별로 같은 기회를 주고, 구역별 선택보다 val 1주에 덜 맞춰짐(사용자 결정) |
| (v2.8) r1의 '나빠지지 않음'은 구역마다, LSTM은 시드 평균 지표로 판정 | 세 구역 합계 / 세 시드 모두 | 한 구역의 놓침 증가를 다른 구역이 가리지 않게 함. 시드 평균은 3단계 결정 지표와 같은 방식(사용자 결정) |
| (v2.9) H 후보 0·30·60·120분, 켜기 기준 1.0·0.9·0.8θ, 세 거리 모두, 지표 따로 보고 + 두 기준선, 지연 0·10분 둘 다 | H·기준 일부만, 60분만, 가중 합 | 5161의 같은 날 안 틈(20~30분)부터 큰 H까지, 놓침이 더 비싼 목적상 낮춘 기준까지 봄. 3단계와 같은 거리 구성. 비용 비율을 정할 근거가 없어 가중하지 않음(사용자 결정) |
| (v2.10) 지연 적용 A: 4단계는 예측이 t − h + 1에 나온다고 봄, 지연값마다 따로 선택 | 3단계 시각(t − h) 유지하고 지연 후보를 키움(B), 지연 0분만(C) | t − h 칸의 값은 그 칸이 끝나야 확정됨. t − h 기준이면 10분 지연이 어떤 결과도 바꾸지 않음. 지연은 모델이 고르는 값이 아니라 환경 가정(사용자 결정) |
| (v2.11) 선택 규칙 r1 → s1(켜 둔 시간 여유 20% 안에서 놓침 최소) | 가중 합(s2), 놓침 목표치(s3), 놓침 최소(s4) | 후보의 켜짐 칸은 늘 기준의 상위 집합이라 r1은 늘 기준을 골라 놓침과 켜 둔 시간의 거래를 하지 않았음(구조적 결함, 구현 후 발견). s1은 r1을 뒤집은 모양으로 '켜 두는 시간을 3단계보다 최대 20% 더 쓴다'고 설명 가능. 규칙과 숫자는 각 규칙이 고를 조합을 보기 전에 정함(사용자 결정) |
| (사후 분석, test 뒤) 놓침 비용 비율 w는 하나로 정하지 않고 w 범위별 1위를 민감도 분석으로 보고 | w 하나(예: 3·5·10)로 순위 확정 | test를 본 뒤라 w를 고르면 결과에 맞춘 선택이 될 수 있음. 60분 뒤·지연 0 합계 기준 val: w<1.6 지난주 같은 시각, 1.6~20.9 LSTM, >20.9 ARIMA / test: w<0.4 마지막 관측값, 0.4~4.7 LSTM, >4.7 지난주 같은 시각. 지난주 같은 시각의 놓침은 val 910분·test 30분으로 불안정(사용자 결정) |
| (v2) test(F-09) 형식 개정은 4단계 변경 통제로 미룸 |
| (v2.12) F-09 개정: 세 거리 × 다섯 계열, 두 기준 대비 구간, LSTM은 시드별 절대오차 평균, 결정 지표·켜기/끄기 포함, 실험 선택은 val MAE 규칙 유지, `final.yaml`은 사람이 작성 | 60분만·10분만 / lag-1008 제외 / lag-144만 기준 / 시드 평균 예측 / 결정 지표로 선택 / 자동 생성 | 목적 단계 2~4를 test로 확인. 가장 강한 짧은 거리 기준(lag-1)에도 이기는지 봄. 보고 MAE와 같은 양의 구간. 미리 정한 규칙을 바꾸지 않음. test에 무엇이 들어가는지 커밋으로 남김(사용자 결정) | 지금 개정 | test는 1~4 확정 뒤 한 번. 시뮬레이션 설계 전에 형식을 정하면 다시 바뀜 |
...
| 9 | F-09 | M-17, M-14 / I-05 / 테스트(픽스처로만: LOCK·원자성·confirm·dirty·선택 규칙·부트스트랩 재현) | 완료(v1 형식, v2.12 개정은 25) | pytest 162 통과(픽스처 full 단계로 검증: LOCK·E-4005·--confirm·dirty·선택 규칙·D-11 누락·원자성·재학습 없음). 구현 중 발견한 버그: final.yaml 키 순서(알파벳)대로 처리해 부트스트랩 기준이 lag-144가 아니게 되던 문제 → 항상 FAMILIES 순서로 처리하도록 수정, 회귀 테스트 유지. 실제 test는 순서 12(사용자 승인 필수) |
...
| 12 | 운영 | 실제 `test --confirm` 1회 | 완료 | 2026-09-23 사용자 지시("설정 동결, test 실행"). 커밋 d163b69의 `final.yaml`(45칸)로 1회 실행, 10초, `results/test/LOCK` 생성. 구역 평균 상대 MAE(10/30/60분): LSTM 0.284/0.387/0.521, ARIMA 0.284/0.430/0.687, 마지막 관측값 0.303/0.514/0.914. 60분 뒤 켜기/끄기(지연 0, 세 구역 합): LSTM 놓침 407분·켜 둔 시간 5,423분, ARIMA 280·6,100, 지난주 같은 시각 30·7,200, 미래를 아는 경우 0·5,100. test 혼잡 구간 19개(4/10/5) |
| 13 | F-10 | README 결과·한계 정리 | 완료 | 2026-09-23. (이후: 생성 스크립트를 `scripts/make_readme.py`로 저장소에 넣고, README가 스크립트 출력과 같은지 테스트) `README.md`: test 결과(거리별 상대 MAE, 마지막 관측값 대비 95% 구간, 60분 뒤 켜기/끄기), 데이터·방법, 한계 5항목, 재현 절차(명령·시드·기대 지표), 실험 58개 표(폴더 링크·바꾼 요소·이유). 수치는 결과 파일에서 스크립트로 생성. 테스트(상대 링크가 모두 존재, 실험 링크 58개, 한계 5항목) 추가, pytest 263 통과 |
...
| 23 | F-14 | (v2.10) M-19, M-15(`switching` 키 검증), M-12(D-16 표) / 테스트(손계산 예제, 3단계 경보와 일치, r1, 시드 평균, E-2001, 재학습 없음) | 완료 | 2026-09-23. pytest 256 통과. `results`가 `switching.csv`(1,080 조합 + 기준선 6행)·`switching.md`를 7초에 생성, 재학습 없음. **발견한 설계 문제(사용자 결정 대기):** 최소 유지를 늘리거나 켜기 기준을 낮추면 켜짐 칸이 기준 조합의 상위 집합이 되므로 켜 둔 시간은 항상 기준 이상, 놓친 시간은 항상 기준 이하. 따라서 r1(놓침이 나빠지지 않는 조합 중 켜 둔 시간 최소)은 구조적으로 항상 기준(H 0분·1.0θ)을 고름. 실제 결과도 15개 (계열, 거리) × 지연 2개 모두 기준 선택. 또 60분 뒤 예측에서는 지연 10분이 결과를 바꾸지 않음(L = 0) |
| 24 | F-14 개정 | (v2.11) M-19 `choose_s1`(r1 대체), M-12 md 설명 / 테스트(여유 안 놓침 최소, 구역마다 여유, 동률 순서, 기준 유지) | 완료 | 2026-09-23 사용자 결정. pytest 256 통과. val 결과: 모든 계열에서 켜기 기준은 1.0θ가 선택됨(0.9·0.8θ는 적어도 한 구역에서 켜 둔 시간이 20%를 넘음). 60분 뒤 예측에서 LSTM은 기준(H 0분) 그대로(놓친 시간 423분, 세 구역 합·시드 평균), ARIMA는 H 30분(420→400분, 켜 둔 시간 +1%), 마지막 관측값 H 30분(940→880), 지난주 같은 시각 H 60분(1,230→910). 60분 뒤에서는 지연 10분이 결과를 바꾸지 않음 |
| 25 | F-09 개정 | (v2.12) M-14(final.yaml 거리 단계·lag-1008, 두 기준 구간, 시드별 절대오차, 결정 지표, 켜기/끄기), M-12(공용 계산 함수) / 테스트(픽스처: 새 형식 결과 표·검증·원자성·재학습 없음·F-11) | 완료 | 2026-09-23 사용자 결정. pytest 261 통과(픽스처: 거리 1·3, 계열 4개로 새 형식 결과 표·두 기준 구간·LSTM 시드별 절대오차 구간·val 선택 켜기/끄기 적용·E-4006 7종·E-2001·원자성·재학습 없음·F-11 평균 행). 실제 상수는 거리 (1, 3, 6), 계열 5개(테스트로 고정). 이전 코드의 버그도 함께 해소: ARIMA·naive test 예측이 거리를 넘기지 않았음(h=1 전용이라 드러나지 않았음). 실제 test 실행(12)은 `configs/final.yaml` 작성 후 "설정 동결, test 실행" 지시가 있을 때 |
| 26 | 사후 분석 | 놓침 비용 비율 민감도(README 결과 요약, 요약 문서·PDF) | 완료 | 2026-09-23 사용자 결정. test 결과를 본 뒤 추가한 분석임을 명시. 새 test 실행 없음, 저장된 결과 파일에서 계산 |
