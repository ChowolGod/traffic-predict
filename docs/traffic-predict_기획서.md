...
## 다음 세션에게
- 현재 단계: 구현 중. 1(골격) 완료, 2(F-01) 실제 데이터 확인만 남음, 3(F-02)·4(F-03) 완료, 다음은 5(F-04)
- 확정된 결정:
...
| 2 | F-01 | M-01, M-02, 진단 보고 / I-01의 캐시 부분 / 테스트(픽스처 합산값, `.gz`, E-1001~1003) → **dev 원본(11/03~11/18)으로 전제 A1~A4 확인** | 진행 중 | 코드·테스트 완료(pytest 46 통과). 343MB 합성 파일 캐시 4.2초·최대 약 760MB. **남은 것: 실제 dev 원본(11/03~11/18)으로 `prepare --phase dev` → `data/interim/inspect.json`으로 A1~A4 확인**. 전제가 틀리면 변경 통제 |
...
| 5 | F-04 | M-10 / 테스트(손계산 예제 1e-9, 채운 시각 제외, E-4004, rel_mae) | 대기 | |
